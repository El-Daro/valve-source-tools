...
function Get-VmfRegex {
	...
	return @{
		...
		comment = "(?nx:				# No implicit captures, exclude whitespaces and allow comments like this one
		^(?<comment>\\\\.*))";			# Matches '\\' comment symbols and whatever comes after
		...
		className = "(?nx:
		(?<className>[^\t\r\n\\{} ]+)	# The class name. Whitespaces are not allowed. '\' is a comment character
		)"

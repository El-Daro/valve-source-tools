	...
	PROCESS {
		...
		try {
			...
			if (-not $Silent.IsPresent) {
				...
				OutLog -Property "Input filters"		-Value $("{0} / {1}" -f $counterStripper["filter"], $counterStripper["modes"])		-Path $LogFile
				OutLog -Property "Input adds"			-Value $("{0} / {1}" -f $counterStripper["add"], $counterStripper["modes"])			-Path $LogFile
				OutLog -Property "Input modifies"		-Value $("{0} / {1}" -f $counterStripper["modify"], $counterStripper["modes"])		-Path $LogFile
				# OutLog -Property "Merged filters"		-Value $("{0} / {1}" -f $mergesCount["filter"], $counterStripper["modes"])			-Path $LogFile
				# OutLog -Property "Merged adds"			-Value $("{0} / {1}" -f $mergesCount["add"], $counterStripper["modes"])				-Path $LogFile
				# OutLog -Property "Merged modifies"		-Value $("{0} / {1}" -f $mergesCount["modify"], $counterStripper["modes"])			-Path $LogFile
				...
				# 	OutLog -Property "Merges failed"	-Value $("{0} / {1}" -f $mergesCount["failed"], $counterStripper["modes"])				-Path $LogFile
				# }

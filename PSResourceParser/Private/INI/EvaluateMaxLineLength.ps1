function EvaluateIniMaxLineLength {
	...
	foreach ($key in $Settings[$section].Keys) {
		...
		$lengthMax = [math]::min( [math]::max(($key.Length + $Settings[$Section][$key].Length + 5), $lengthMax), 100 )
	}
	if ($lengthMax % 4 -eq 0) {
		# We need at least some space before the in-line comment
		$additionalLength = 4
	}

...
<#
	.SYNOPSIS
	Validates a block in .vdf file.

	.DESCRIPTION
	Takes an array of strings representing a .vdf formatted content as an input and transforms them into an ordered hashtable.

	.PARAMETER Lines
	An array of .vdf formatted strings. Use `Get-Content $pathToVdf` to obtain a proper array.
	...
	.PARAMETER Depth
	Current depth (ref). Starts at 0, but increases for the very first block, which every .vdf file usually starts with.

	.PARAMETER Regex
	VDF-specific regex hashtable.
	...
	3. If we're hitting a key-value pair, validate it and to the hashtable.
	  ...
	  3.2. If there is no bracket next line, terminate the execution (badly formatted .vdf).
